...
/**
 * Strategy for fetching commits and files from a GitHub Pull Request.
 * Uses the /repos/{owner}/{repo}/pulls/{number}/commits and /pulls/{number}/files endpoints.
 */
@Component
public class PullRequestFetchStrategy implements GitHubFetchStrategy {
    ...
    public PullRequestFetchStrategy(RestTemplate restTemplate, ObjectMapper objectMapper) {
    ...
    public List<CommitInfo> fetchCommits(GitHubContext context, String token) {
        String url = String.format(
            "%s/repos/%s/%s/pulls/%s/commits",
            githubBaseUrl, context.getOwner(), context.getRepo(), context.getPrNumber()
        );
